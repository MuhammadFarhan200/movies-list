...
const MovieDetail = () => {
  ...
  return (
    <>
      <Navbar />
      <div className={`relative w-full h-[350px]`} style={{ backgroundImage: `url(${import.meta.env.VITE_IMG_URL}/${movie.backdrop_path})`, backgroundSize: 'cover', backgroundPosition: 'top center' }}>
        ...
        <img src={`${import.meta.env.VITE_IMG_URL}/${movie.poster_path}`} alt={movie.title}  
          className='rounded-lg w-48 my-3 text-slate-200 absolute top-1/2 left-1/2 lg:left-16 transform -translate-x-1/2 lg:translate-x-0 -translate-y-1/2'
        />
        <div className='hidden lg:block absolute top-1/3 left-1/2 transform -translate-x-1/2 -translate-y-1/3'>
          <h1 className='text-sky-500 text-5xl font-semibold text-center'>{movie.title}</h1>
        </div>
        <div className='hidden lg:block absolute bottom-12 left-1/2 tranfotm -translate-x-1/2'>
          <div className='container mx-aut px-5 sm:px-10'>
            <table className='table-auto text-slate-200 text-xl'>
              <tbody>
                <tr>
      ...
      <div className='container mx-auto p-5 sm:p-10'>
        ...
        <div className='bg-slate-800 rounded-xl p-4 mt-7 mb-10'>
          ...
          <p className='text-slate-200 text-lg my-3 overflow-hidden whitespace-nowrap'>Official Site:&nbsp; 
            <Link to={movie.homepage} className='text-sky-500 text-ellipsis' target='_blank'>{movie.homepage}</Link>
          </p>
        ...
        <button onClick={() => history.back()} className='block w-fit bg-slate-800 outline-none hover:text-cyan-400 focus:ring-2 focus:ring-cyan-500 text-slate-200 px-4 py-3 rounded-lg my-5 ms-auto'>
